...
def id3_calculation(Xtrn, ytrn, Xtst, ytst, attribute_value_pairs):
    test_errors_dict = {}
    train_errors_dict = {}
    ...
    for i in range(10):
        ...
        test_errors_dict[i] = tst_err
        
        y_pred = [predict_example(x, decision_tree) for x in Xtrn]
        trn_err = compute_error(ytrn, y_pred)
        train_errors_dict[i] = trn_err
         
    plt.figure()
    plt.plot(list(train_errors_dict.keys()), list(train_errors_dict.values()), marker='o', linewidth=3, markersize=12)
    plt.plot(list(test_errors_dict.keys()), list(test_errors_dict.values()), marker='s', linewidth=3, markersize=12)
    
    plt.xlabel('Tree Depth', fontsize=18)
    plt.ylabel('Training/Testing error', fontsize=18)
    
    plt.xticks(list(train_errors_dict.keys()), fontsize=14)
    plt.legend(['Training Error', 'Testing Error'], fontsize=18)
...
if __name__ == '__main__':
    ...
    print("\nPart - B")
    # Dataset 1
    id3_calculation(Xtrn, ytrn, Xtst, ytst, attribute_value_pairs)
    
    # Training dataset 2
    M2 = np.genfromtxt('./monks_data/monks-2.train', missing_values=0, skip_header=0, delimiter=',', dtype=int)
    y2trn = M2[:, 0]
    X2trn = M2[:, 1:]
    
    # Test dataset 2
    M2 = np.genfromtxt('./monks_data/monks-2.test', missing_values=0, skip_header=0, delimiter=',', dtype=int)
    y2tst = M2[:, 0]
    X2tst = M2[:, 1:]    
    
    attribute_value_pairs = []  
    for i in range(X2trn.shape[1]):
        X = np.array(M2[:,i+1])
        for j in range(len(np.unique(X,return_counts = True)[1])):
            attribute_value_pairs.append((i, np.unique(X)[j]))
    id3_calculation(X2trn, y2trn, X2tst, y2tst, attribute_value_pairs)
    
    # Training dataset 3
    M3 = np.genfromtxt('./monks_data/monks-3.train', missing_values=0, skip_header=0, delimiter=',', dtype=int)
    y3trn = M3[:, 0]
    X3trn = M3[:, 1:]
    
    # Test dataset 3
    M3 = np.genfromtxt('./monks_data/monks-3.test', missing_values=0, skip_header=0, delimiter=',', dtype=int)
    y3tst = M3[:, 0]
    X3tst = M3[:, 1:]    
 
    attribute_value_pairs = []  
    for i in range(X3trn.shape[1]):
        X = np.array(M3[:,i+1])
        for j in range(len(np.unique(X,return_counts = True)[1])):
            attribute_value_pairs.append((i, np.unique(X)[j]))
    id3_calculation(X3trn, y3trn, X3tst, y3tst, attribute_value_pairs)
    ...
    for val in range(1,6,2):
        ...
        print("\nDepth -", val)
        decision_tree = id3(Xtrn, ytrn, attribute_value_pairs, max_depth=val)
        y_pred = [predict_example(x, decision_tree) for x in Xtst]
        tst_err = compute_error(ytst, y_pred)
    ...
    print("\nPart - D")
    # Depth = 1
    classifier1 = tree.DecisionTreeClassifier(criterion='entropy', max_depth=1)
    classifier1.fit(Xtrn, ytrn)
    ypred = classifier1.predict(Xtst)
    print("\n Depth = 1 \n", confusion_matrix(ytst, ypred))
    
    # Depth = 3
    classifier3 = tree.DecisionTreeClassifier(criterion='entropy', max_depth=3)
    classifier3.fit(Xtrn, ytrn)
    ypred = classifier3.predict(Xtst)
    print("\n Depth = 3 \n", confusion_matrix(ytst, ypred)) 
    
    # Depth = 5
    classifier5 = tree.DecisionTreeClassifier(criterion='entropy', max_depth=5)
    classifier5.fit(Xtrn, ytrn)
    ypred = classifier5.predict(Xtst)
    print("\n Depth = 5 \n", confusion_matrix(ytst, ypred))
    ...
    Xtrn, Xtst, ytrn, ytst = train_test_split(X, y, test_size=0.20)
    
    classifier = DecisionTreeClassifier(criterion='entropy')
    classifier.fit(Xtrn, ytrn)
    y_pred = classifier.predict(Xtst)
    print("\nConfusion Matrix for Caesarian dataset using scikit-learn:")
    print(confusion_matrix(ytst, y_pred))
    ...
    decision_tree = id3(np.array(Xtrn), np.array(ytrn), attribute_value_pairs, max_depth=5)
    pretty_print(decision_tree)
    dot_str = to_graphviz(decision_tree)
    render_dot_file(dot_str, './my_learned_tree_parte')
    
    y_pred = [predict_example(x, decision_tree) for x in np.array(Xtst)]
    tst_err = compute_error(ytst, y_pred)
    print("\n")
    print("Confusion Matrix for Caesarian dataset using ID3 algorithm:")
    print(confusion_matrix(ytst, y_pred))
